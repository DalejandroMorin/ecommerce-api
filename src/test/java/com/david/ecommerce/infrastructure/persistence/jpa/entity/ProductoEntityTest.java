...
class ProductoEntityTest extends AbstractIntegrationTest {
    ...
    void persistir_ProductoConTodosLosCampos_RetornaProductoCreado() {
        ...
        entity.setCategoria(Categoria.ELECTRONICA);
        ...
        assertThat(found.getCategoria()).isEqualTo(Categoria.ELECTRONICA);
    }
    ...
    @Test
    @DisplayName("Verificar que todas las Categoria del dominio existen")
    void enumSync_TodasLasCategoriasMapean() {
        for (Categoria domainCat : Categoria.values()) {
            assertThat(domainCat.name()).isEqualTo(domainCat.name());
        }
    }
    ...
    void columnas_ConRestricciones_SePersistenCorrectamente() {
        ...
        entity.setCategoria(Categoria.OTROS);
