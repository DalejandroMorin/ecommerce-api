...
class ProductoMapperTest {
    ...
    void roundtrip_DominioAEntidadADominio_PreservaCampos() {
        ...
        domain.setCategoria(Producto.Categoria.ELECTRONICA);
        ...
        assertThat(entity.getCategoria()).isEqualTo(ProductoEntity.Categoria.ELECTRONICA);
        ...
        assertThat(result.getCategoria()).isEqualTo(Producto.Categoria.ELECTRONICA);
    ...
    @Test
    @DisplayName("Todas las categorias del dominio mapean correctamente al entity enum mediante valueOf")
    void todasLasCategorias_MapeanCorrectamente() {
        for (Producto.Categoria domainCat : Producto.Categoria.values()) {
            var domain = new Producto();
