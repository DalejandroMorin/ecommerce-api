...
class PedidoEntityTest extends AbstractIntegrationTest {
    ...
    void prePersist_EstableceFechaYEstadoPorDefecto() {
        ...
        assertThat(found.getEstado()).isEqualTo(PedidoEntity.EstadoPedido.PENDIENTE);
    }

    @Test
    @DisplayName("Verificar que todos los EstadoPedido del dominio existen en la entidad (EC-1)")
    void enumSync_DominioAEntidad_TodosLosEstadosMapean() {
        for (com.david.ecommerce.domain.pedido.Pedido.EstadoPedido domainEstado
                : com.david.ecommerce.domain.pedido.Pedido.EstadoPedido.values()) {
            var entityEstado = PedidoEntity.EstadoPedido.valueOf(domainEstado.name());
            assertThat(entityEstado.name()).isEqualTo(domainEstado.name());
        }
    }
